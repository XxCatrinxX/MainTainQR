...
<head>
    ...
    <title>Diagnóstico Listo — {{ $orden->folio }}</title>
</head>
                    ...
                    <tr>
                        <td style="background:#ffffff; padding: 2rem 2.5rem;">
                            ...
                            <p style="color:#6b7280; font-size:0.95rem; line-height:1.6; margin: 0 0 1.5rem 0;">
                                ...
                                ha sido diagnosticado. A continuación encontrarás el resumen:
                            </p>
                            ...
                            <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="border:1px solid #e5e7eb; border-radius:8px; overflow:hidden; margin-bottom:1.5rem;">
                                <tr style="background:#f9fafb;">
                                    <td style="padding:0.75rem 1rem; color:#6b7280; font-size:0.8rem; font-weight:600; text-transform:uppercase; letter-spacing:0.05em;">Concepto</td>
                                    <td style="padding:0.75rem 1rem; color:#6b7280; font-size:0.8rem; font-weight:600; text-transform:uppercase; letter-spacing:0.05em; text-align:right;">Costo</td>
                                </tr>
                                <tr>
                                    <td style="padding:0.75rem 1rem; color:#374151; border-top:1px solid #f3f4f6;">Mano de Obra</td>
                                    <td style="padding:0.75rem 1rem; color:#374151; border-top:1px solid #f3f4f6; text-align:right; font-weight:600;">${{ $manoObra }}</td>
                                </tr>
                                @if($totalPiezas != '0.00')
                                <tr>
                                    <td style="padding:0.75rem 1rem; color:#374151; border-top:1px solid #f3f4f6;">Repuestos / Materiales</td>
                                    <td style="padding:0.75rem 1rem; color:#374151; border-top:1px solid #f3f4f6; text-align:right; font-weight:600;">${{ $totalPiezas }}</td>
                                </tr>
                                @endif
                                <tr style="background:#111827;">
                                    <td style="padding:0.85rem 1rem; color:#ffffff; font-weight:700; font-size:1rem;">Total Estimado</td>
                                    <td style="padding:0.85rem 1rem; color:#ffffff; font-weight:800; font-size:1.1rem; text-align:right;">${{ $total }}</td>
                                </tr>
                            </table>

                            {{-- PHOTOS --}}
                            @if($orden->evidencias && $orden->evidencias->count() > 0)
                            <p style="color:#374151; font-weight:600; font-size:0.875rem; margin:0 0 0.75rem 0;">Evidencia fotográfica adjunta ({{ $orden->evidencias->count() }} foto{{ $orden->evidencias->count() > 1 ? 's' : '' }}):</p>
                            ...
                            <p style="color:#374151; font-size:0.95rem; line-height:1.6; margin:0 0 1.5rem 0;">
                                ¿Qué deseas hacer con tu equipo? Por favor elige una opción:
                            </p>
